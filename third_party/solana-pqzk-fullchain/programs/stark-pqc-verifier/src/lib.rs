...
#[cfg(feature = "phase2-instrumentation")]
pub use phase2::{Phase2InitChatMsg, Phase2WriteChatMsgChunk};
...
#[cfg(feature = "phase2-instrumentation")]
pub mod __client_accounts_phase2_init_chat_msg { pub use crate::Phase2InitChatMsg; }
#[cfg(feature = "phase2-instrumentation")]
pub mod __client_accounts_phase2_write_chat_msg_chunk { pub use crate::Phase2WriteChatMsgChunk; }
...
pub mod stark_pqc_verifier {
    ...
    /// Phase-2-only helper: initializes a ChatMsg directly so the real verify_stark path can be benchmarked in isolation.
    #[cfg(feature = "phase2-instrumentation")]
    pub fn phase2_init_chat_msg(
        ...
        phase2::handle_phase2_init_chat_msg(ctx, recipient, cipher_len, kem_len, nonce, slot, payload_len)
    }

    /// Phase-2-only helper: writes a chunk into the ChatMsg payload directly.
    #[cfg(feature = "phase2-instrumentation")]
    pub fn phase2_write_chat_msg_chunk(
        ctx: Context<Phase2WriteChatMsgChunk>,
        offset: u32,
        data: Vec<u8>,
    ) -> Result<()> {
        phase2::handle_phase2_write_chat_msg_chunk(ctx, offset, data)
    }
}

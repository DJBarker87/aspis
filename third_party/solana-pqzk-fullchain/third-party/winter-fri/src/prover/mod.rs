...
const FRI_PROOF_LAYOUT_RAW_ROWS: u8 = 0;
const FRI_PROOF_LAYOUT_RAW_ROWS_PLUS_INTERPOLANTS: u8 = 1;
static FRI_PROOF_LAYOUT: AtomicU8 = AtomicU8::new(FRI_PROOF_LAYOUT_RAW_ROWS);

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub struct FriProofLayerLayout {
    pub include_interpolants: bool,
}

impl FriProofLayerLayout {
    pub const RAW_ROWS: Self = Self {
        include_interpolants: false,
    };
    pub const RAW_ROWS_PLUS_INTERPOLANTS: Self = Self {
        include_interpolants: true,
    };
}

pub fn set_fri_proof_layer_layout(layout: FriProofLayerLayout) {
    FRI_PROOF_LAYOUT.store(
        if layout.include_interpolants {
            FRI_PROOF_LAYOUT_RAW_ROWS_PLUS_INTERPOLANTS
        } else {
            FRI_PROOF_LAYOUT_RAW_ROWS
        },
        Ordering::Relaxed,
    );
}

fn current_fri_proof_layer_layout() -> FriProofLayerLayout {
    match FRI_PROOF_LAYOUT.load(Ordering::Relaxed) {
        FRI_PROOF_LAYOUT_RAW_ROWS_PLUS_INTERPOLANTS => {
            FriProofLayerLayout::RAW_ROWS_PLUS_INTERPOLANTS
        }
        _ => FriProofLayerLayout::RAW_ROWS,
    }
}
    ...
    V: VectorCommitment<H>,
{
    ...
    pub fn build_proof(&mut self, positions: &[usize]) -> FriProof {
        ...
        let layout = current_fri_proof_layer_layout();
        ...
        if !self.layers.is_empty() {
            ...
            let mut domain_generator =
                <E::BaseField as StarkField>::get_root_of_unity(domain_size.ilog2());
            ...
            for i in 0..self.layers.len() {
                ...
                domain_generator = domain_generator.exp_vartime((folding_factor as u32).into());
            }
        }
...
fn query_layer<E: FieldElement, H: Hasher, V: VectorCommitment<H>, const N: usize>(
    ...
    if layout.include_interpolants {
        let folding_roots = (0..N)
            .map(|i| {
                domain_generator.exp_vartime(((domain_size / N * i) as u64).into())
            })
            .collect::<Vec<_>>();
        #[rustfmt::skip]
        let xs = positions.iter().map(|&i| {
        ...
        let interpolants = polynom::interpolate_batch(&xs, &queried_values);
        FriProofLayer::new_with_interpolants::<_, _, V, N>(queried_values, interpolants, proof.1)
    } else {

...
/// Query payload returned for a single FRI layer.
pub struct LayerQueries<E: FieldElement, const N: usize> {
    pub values: Vec<[E; N]>,
    pub interpolants: Option<Vec<[E; N]>>,
}
...
pub trait VerifierChannel<E: FieldElement> {
    ...
    fn read_layer_queries<const N: usize>(
    ...
    ) -> Result<LayerQueries<E, N>, VerifierError> {
        let layer_proof = self.take_next_fri_layer_proof();
        let mut layer_queries = self.take_next_fri_layer_queries();
        let expected_elements = positions.len() * N;
        if layer_queries.len() != expected_elements && layer_queries.len() != expected_elements * 2 {
            return Err(VerifierError::NumPositionEvaluationMismatch(
                positions.len(),
                layer_queries.len() / N,
            ));
        }
        let interpolants = if layer_queries.len() == expected_elements * 2 {
            Some(layer_queries.split_off(expected_elements))
        } else {
            None
        };
        ...
        Ok(LayerQueries {
            values: leaf_values.to_vec(),
            interpolants: interpolants.map(|values| group_slice_elements(&values).to_vec()),
        })
    }

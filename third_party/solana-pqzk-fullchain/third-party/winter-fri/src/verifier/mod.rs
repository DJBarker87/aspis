    ...
    V: VectorCommitment<H>,
{
    ...
    fn verify_generic<const N: usize>(
        ...
        for depth in 0..self.options.num_fri_layers(self.domain_size) {
            ...
            let layer_queries = channel.read_layer_queries(&position_indexes, &layer_commitment)?;
            let query_values = get_query_values::<E, N>(
                &layer_queries.values,
                &positions,
                &folded_positions,
                domain_size,
            );
            ...
            let row_polys = layer_queries
                .interpolants
                .unwrap_or_else(|| interpolate_rows(&xs, &layer_queries.values));

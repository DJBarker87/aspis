...
impl FriProofLayer {
    ...
    /// Creates a new proof layer from the specified query values, pre-packaged interpolants, and
    /// the corresponding batch opening proof.
    ///
    /// Raw values remain the commitment-bound payload. Interpolants are a verifier-side sidecar
    /// used to skip row interpolation when the verifier supports it.
    pub(crate) fn new_with_interpolants<
        E: FieldElement,
        H: Hasher,
        V: VectorCommitment<H>,
        const N: usize,
    >(
        ...
        assert_eq!(
            query_values.len(),
            interpolants.len(),
            "interpolants must match query count"
        );

        let mut value_bytes = Vec::with_capacity(E::ELEMENT_BYTES * N * query_values.len());
        value_bytes.write_many(&query_values);

        let mut interpolant_bytes = Vec::with_capacity(E::ELEMENT_BYTES * N * interpolants.len());
        interpolant_bytes.write_many(&interpolants);
        ...
        FriProofLayer {
            values: value_bytes,
            interpolants: interpolant_bytes,
            paths: proof_bytes,
        }
    }
    ...
    pub fn size(&self) -> usize {
        // +4 for length of values, +4 for length of interpolants, +4 for length of paths
        self.values.len() + 4 + self.interpolants.len() + 4 + self.paths.len() + 4
    }
        ...
        V: VectorCommitment<H>,
    {
        ...
        if !self.interpolants.is_empty() {
            if self.interpolants.len() != self.values.len() {
                return Err(DeserializationError::InvalidValue(format!(
                    "number of interpolant bytes ({}) must match query value bytes ({})",
                    self.interpolants.len(),
                    self.values.len(),
                )));
            }
            let mut reader = SliceReader::new(&self.interpolants);
            for _ in 0..num_queries {
                let mut coeffs = reader.read_many(folding_factor)?;
                query_values.append(&mut coeffs);
            }
            if reader.has_more_bytes() {
                return Err(DeserializationError::UnconsumedBytes);
            }
...
impl Serializable for FriProofLayer {
    ...
    fn write_into<W: ByteWriter>(&self, target: &mut W) {
        ...
        // write optional interpolant bytes
        target.write_u32(self.interpolants.len() as u32);
        target.write_bytes(&self.interpolants);
...
impl Deserializable for FriProofLayer {
    ...
    fn read_from<R: ByteReader>(source: &mut R) -> Result<Self, DeserializationError> {
        ...
        // read optional interpolants
        let num_interpolant_bytes = source.read_u32()?;
        let interpolants = source.read_vec(num_interpolant_bytes as usize)?;
